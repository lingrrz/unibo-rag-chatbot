"""
Unibo Website Scraper for Agentic RAG
BFS crawler for www.unibo.it / corsi.unibo.it / www.eng.unibo.it.
Converts HTML pages to Markdown, extracts text from course PDFs,
and injects YAML frontmatter (metadata) for LLM/Agent routing.

Usage:
  python scraper.py    # default seeds, 5000 pages
  python scraper.py --preset all-target-courses --max-pages 1500
...
# Predefined course sets targeting specific MsC programs.
# Root URLs are provided; the crawler will automatically append COURSE_SUBPAGES 
# (like /faculty, /programme) to explore professor profiles and syllabuses.
PRESET_URLS: dict[str, list[str]] = {
    "ai-msc": ["https://corsi.unibo.it/2cycle/artificial-intelligence"],
    "cs-msc": ["https://corsi.unibo.it/2cycle/ComputerScienceEngineering"],
    "automation": ["https://corsi.unibo.it/2cycle/AutomationEngineering"],
    "aerospace": ["https://corsi.unibo.it/2cycle/AerospaceEngineering"],
    "physics-earth": ["https://corsi.unibo.it/2cycle/PhysicsEarthOceanAtmosphere"],
    "digital-humanities": ["https://corsi.unibo.it/2cycle/DigitalHumanitiesKnowledge"],
    "legal-studies": ["https://corsi.unibo.it/2cycle/legal-studies"],
    "gioca": ["https://corsi.unibo.it/2cycle/gioca"],
    
    "all-target-courses": [
        "https://corsi.unibo.it/2cycle/artificial-intelligence",
        "https://corsi.unibo.it/2cycle/ComputerScienceEngineering",
        "https://corsi.unibo.it/2cycle/AutomationEngineering",
        "https://corsi.unibo.it/2cycle/AerospaceEngineering",
        "https://corsi.unibo.it/2cycle/PhysicsEarthOceanAtmosphere",
        "https://corsi.unibo.it/2cycle/DigitalHumanitiesKnowledge",
        "https://corsi.unibo.it/2cycle/legal-studies",
        "https://corsi.unibo.it/2cycle/gioca"
    ]
}

# Boost priority for professor profiles and study/research pages
    ...
    for pdf_url in sorted(pdf_urls):
        ...
        if text:
            with open(outpath, "w", encoding="utf-8") as f:
                # Inject YAML Frontmatter for PDF syllabus
                f.write("---\n")
                f.write(f"url: {pdf_url}\n")
                f.write("type: syllabus_pdf\n")
                f.write("---\n\n")
                f.write(f"{text}")
...
async def scrape_page(
    ...
    try:
        ...
        if filepath.exists():
            ...
            # Dynamically determine page type for Agentic metadata filtering
            page_type = "general_page"
            if "/sitoweb/" in url:
                page_type = "professor_profile"
            elif urlparse(url).netloc == "corsi.unibo.it":
                page_type = "course_info"
            
            if shallow:
                page_type = "shallow_events_or_news"

            with open(filepath, "w", encoding="utf-8") as f:
                # Inject YAML Frontmatter for standard web pages
                f.write("---\n")
                f.write(f"url: {url}\n")
...
def load_checkpoint(output_dir: Path) -> dict | None:
    ...
    for md_file in output_dir.glob("*.md"):
        if md_file.name.startswith("_"):
            continue
        try:
            # Parse YAML frontmatter to reconstruct visited URLs
            lines = md_file.read_text(encoding="utf-8").split("\n")
            if len(lines) > 2 and lines[0] == "---":
                for line in lines[1:5]:
                    if line.startswith("url: "):
                        visited_urls.append(line[5:].strip())
                        break
        except Exception:
            pass
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="BFS web scraper for the University of Bologna website, optimized for Agentic RAG.",
