...
Stages:
  ...
  6. YAML Preserv.  — Extracts and perfectly preserves YAML Frontmatter
...
def main():
    parser = argparse.ArgumentParser(
        description="Clean scraped Unibo markdown files while preserving YAML metadata.",
...
Examples:
  ...
  python clean.py --min-chars 500
    ...
    for f in md_files:
        ...
        match = yaml_pattern.match(raw)
        
        if match:
            frontmatter = match.group(0).strip()
            body_raw = raw[match.end():]
        else:
            frontmatter = ""
            # Fallback for old files without YAML frontmatter
            lines = raw.split("\n", 1)
            frontmatter = lines[0] if lines[0].startswith("# Source:") else ""
        ...
        candidates.append((f.name, frontmatter, body_clean))
    ...
    for group in hash_groups.values():
        ...
        # Reconstruct the file with Frontmatter + Double Newline + Cleaned Body
        final_content = (keeper_src + "\n\n" + keeper_body).strip() + "\n"
